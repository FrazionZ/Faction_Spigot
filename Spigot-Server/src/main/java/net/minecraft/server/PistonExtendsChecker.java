...
public class PistonExtendsChecker {
    ...
    public boolean a() {
        ...
        if (!BlockPiston.a(iblockdata, this.a, this.c, this.d, false, this.d)) {
        	...
        	if(iblockdata.getBlock() == Blocks.PUMPKIN || iblockdata.getBlock() == Blocks.MELON_BLOCK || iblockdata.getBlock() == Blocks.REEDS) {
        		return false;
        	}
        	
        	else if (iblockdata.o() == EnumPistonReaction.DESTROY) {
                this.f.add(this.c);
                return true;
            } else {
                return false;
