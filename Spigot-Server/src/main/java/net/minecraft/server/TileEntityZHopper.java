...
public class TileEntityZHopper extends TileEntityLootable implements IHopper, ITickable {
    ...
    public TileEntityZHopper() {
        this.items = NonNullList.a(18, ItemStack.a);
        this.f = -1;
    }

    public static void a(DataConverterManager dataconvertermanager) {
        dataconvertermanager.a(DataConverterTypes.BLOCK_ENTITY, (DataInspector) (new DataInspectorItemList(TileEntityZHopper.class, new String[] { "Items"})));
    }
    ...
    public String getName() {
        return this.hasCustomName() ? this.o : "container.z_hopper";
    }
    ...
    private boolean o() {
        if (this.world != null && !this.world.isClientSide) {
            if (!this.J() && BlockZHopper.f(this.v())) {
                boolean flag = false;
                ...
                if (flag) {
                    this.setCooldown(world.spigotConfig.zhopperTransfer); // Spigot
                    this.update();
                    return true;
                }
            }
    ...
    private boolean s() {
        ...
        if (iinventory == null) {
            ...
            EnumDirection enumdirection = BlockZHopper.b(this.v()).opposite();
    ...
    private static boolean a(IHopper ihopper, IInventory iinventory, int i, EnumDirection enumdirection) {
        ...
        if (!itemstack.isEmpty() && b(iinventory, itemstack, i, enumdirection)) {
            ...
            if (event.isCancelled()) {
                ...
                if (ihopper instanceof TileEntityZHopper) {
                    ((TileEntityZHopper) ihopper).setCooldown(ihopper.getWorld().spigotConfig.zhopperTransfer); // Spigot
                } else if (ihopper instanceof EntityMinecartHopper) {
                    ((EntityMinecartHopper) ihopper).setCooldown(ihopper.getWorld().spigotConfig.zhopperTransfer / 2); // Spigot
                }
                return false;
            }
            int origCount = event.getItem().getAmount(); // Spigot
    ...
    private static ItemStack a(IInventory iinventory, IInventory iinventory1, ItemStack itemstack, int i, EnumDirection enumdirection) {
        ...
        if (a(iinventory1, itemstack, i, enumdirection)) {
            ...
            if (flag) {
                if (flag1 && iinventory1 instanceof TileEntityZHopper) {
                    TileEntityZHopper tileentityhopper = (TileEntityZHopper) iinventory1;

                    if (!tileentityhopper.K()) {
                        byte b0 = 0;

                        if (iinventory != null && iinventory instanceof TileEntityZHopper) {
                            TileEntityZHopper tileentityhopper1 = (TileEntityZHopper) iinventory;
                        ...
                        tileentityhopper.setCooldown(tileentityhopper.world.spigotConfig.zhopperTransfer - b0); // Spigot
                    }
                }
    ...
    private IInventory I() {
        EnumDirection enumdirection = BlockZHopper.b(this.v());
    ...
    public static List<EntityItem> a(World world, double d0, double d1, double d2) {
        return world.a(EntityItem.class, new AxisAlignedBB(d0 - 4.5D, d1 - 1.0D, d2 - 4.5D, d0 + 4.5D, d1 + 4.5D, d2 + 4.5D), IEntitySelector.a);
    }
    ...
    public Container createContainer(PlayerInventory playerinventory, EntityHuman entityhuman) {
        this.d(entityhuman);
        return new ContainerZHopper(playerinventory, this, entityhuman);
    }
