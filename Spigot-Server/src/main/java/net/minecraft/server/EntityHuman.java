...
public abstract class EntityHuman extends EntityLiving {
    ...
    {
        return this.isUsingItem() && this.itemInUse.getItem().f(this.itemInUse) == EnumAnimation.BLOCK_SWORD;
    }
    ...
    public float b(IBlockData iblockdata) {
        ...
        if (f > 1.0F) {
            ...
            if (i > 0 && !itemstack.isEmpty() && itemstack.getItem() != Items.FRAZION_HAMMER) {
                f += (float) (i * i + 1);
            }
        }
    ...
    public void attack(Entity entity)
    {
        if (entity.bd())
        {
            if (!entity.t(this)) 
            {
                ...
                i = i + EnchantmentManager.b((EntityLiving) this);
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    if (flag2)
                    {
                        if (i > 0)
                        {
                            entity.f((double)(-MathHelper.sin(this.yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), 0.1D, (double)(MathHelper.cos(this.yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                            this.motX *= 0.6D;
                            this.motZ *= 0.6D;
                        ...
                        if (!itemstack.isEmpty() && object instanceof EntityLiving) 
                        {
                        ...
                        if (entity instanceof EntityLiving) {
                            this.a(StatisticList.y, Math.round(f * 10.0F));
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    boolean flag2 = this.fallDistance > 0.0F && !this.onGround && !this.m_() && !this.isInWater() && !this.hasEffect(MobEffects.BLINDNESS) && !this.isPassenger() && entity instanceof EntityLiving;
                    ...
                    if (!flag2 && !flag1 && this.onGround && d0 < (double) this.cy())
                    {
                    ...
                    if (flag5)
                    {
                        if (i > 0)
                        {
                            if (entity instanceof EntityLiving)
                            {
                                entity.f((double)(-MathHelper.sin(this.yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), 0.08D, (double)(MathHelper.cos(this.yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                            } 
                            else
                            {
                            	entity.f((double)(-MathHelper.sin(this.yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), 0.08D, (double)(MathHelper.cos(this.yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                        ...
                        /*if (flag3) {
                        	float f4 = 1.0F + EnchantmentManager.b((EntityLiving) this) * f;
                            ...
                            while (iterator.hasNext()) {
                                ...
                                if (entityliving != this && entityliving != entity && !this.r(entityliving) && this.h(entityliving) < 9.0D) {
                                	...
                                	if (entityliving.damageEntity(DamageSource.playerAttack(this), f4)) 
                                	{
                                        ...
                                        entityliving.a(this, 0.55F, (double) MathHelper.sin(this.yaw * 0.017453292F), (double) (-MathHelper.cos(this.yaw * 0.017453292F)));
    ...
    public void addExperienceLevel(int levels, boolean noSound) {
    ...
    public void removeExperienceLevel(int levels, boolean noSound) {
        this.expLevel -= levels;
        if (this.expLevel < 0) {
            this.expLevel = 0;
            this.exp = 0.0F;
    ...
    public ItemStack getEquipment(EnumItemSlot slotIn)
    {
        if (slotIn == EnumItemSlot.MAINHAND)
        {
            return this.inventory.getItemInHand();
        }
        else if (slotIn == EnumItemSlot.OFFHAND)
        {
            return this.inventory.extraSlots.get(0);
        }
        else if(slotIn.a() == EnumItemSlot.Function.ARMOR) {
        	return (ItemStack)this.inventory.armor.get(slotIn.b());
        }
        else if(slotIn.a() == EnumItemSlot.Function.TROPHY) {
        	return (ItemStack)this.inventory.trophyInventory.get(slotIn.b());
        }
        else
        {
            return ItemStack.a;
    ...
    public void setSlot(EnumItemSlot slotIn, ItemStack stack)
    {
        if (slotIn == EnumItemSlot.MAINHAND)
        {
            this.a_(stack);
            this.inventory.items.set(this.inventory.itemInHandIndex, stack);
        }
        else if (slotIn == EnumItemSlot.OFFHAND)
        {
            this.a_(stack);
            this.inventory.extraSlots.set(0, stack);
        }
        else if (slotIn.a() == EnumItemSlot.Function.ARMOR)
        {
            this.a_(stack);
            this.inventory.armor.set(slotIn.b(), stack);
        }
        else if (slotIn.a() == EnumItemSlot.Function.TROPHY)
        {
            this.a_(stack);
            this.inventory.trophyInventory.set(slotIn.b(), stack);
        }
    }
    ...
    public boolean hasItemInMainHand(Item legendaryScythe)
    {
    	...
    	return handItem != null && handItem.getItem().equals(legendaryScythe);
    }
