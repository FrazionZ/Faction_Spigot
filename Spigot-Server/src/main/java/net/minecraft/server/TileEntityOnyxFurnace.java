...
public class TileEntityOnyxFurnace extends TileEntityContainer implements ITickable, IWorldInventory {

    private static final int[] a = new int[] {0, 1, 2, 3};
    private static final int[] f = new int[] {4, 5, 6, 7, 8};
    ...
    public static void a(DataConverterManager dataconvertermanager) {
        dataconvertermanager.a(DataConverterTypes.BLOCK_ENTITY, (DataInspector) (new DataInspectorItemList(TileEntityOnyxFurnace.class, new String[] { "Items"})));
    }
    ...
    public void load(NBTTagCompound nbttagcompound) {
        ...
        this.ticksForCurrentFuel = fuelTime((ItemStack) this.items.get(4));
    ...
    public void e()
    {
        ...
        if (!this.world.isClientSide)
        {
            ...
            if ((this.isBurning() || (!itemstack.isEmpty() && !((ItemStack)this.items.get(0)).isEmpty()) || (!itemstack.isEmpty() && !((ItemStack)this.items.get(3)).isEmpty()) || (!itemstack.isEmpty() && !((ItemStack)this.items.get(2)).isEmpty()) || ( !itemstack.isEmpty() && !((ItemStack)this.items.get(1)).isEmpty())))
            {
                if (!this.isBurning() && this.canBurn())
                {
                    ...
                    if (this.isBurning())
                    {
                        ...
                        if (!itemstack.isEmpty())
                        {
                            ...
                            {
                                Item item1 = item.q();
                                this.items.set(4, item1 == null ? ItemStack.a : new ItemStack(item1));
                            }
                        }
                    }
                }
            ...
            {
                flag1 = true;
                BlockOnyxFurnace.a(this.isBurning(), this.world, this.position);
            }
        }
    ...
    {
        if (yolo(0) && yolo(1) && yolo(2) && yolo(3))
        {
            return false;
        } 
        else
        {
        	...
        	else if (itemstack.isEmpty() && itemstackA.isEmpty() && itemstackB.isEmpty() && itemstackC.isEmpty())
            {
                return false;
            }
            else
            {
                ItemStack itemstack1 = (ItemStack) this.items.get(5);
                
                ItemStack itemstackA1 = (ItemStack) this.items.get(6);
                
                ItemStack itemstackB1 = (ItemStack) this.items.get(7);
                
                ItemStack itemstackC1 = (ItemStack) this.items.get(8);
                    
                if (itemstack1.isEmpty() && itemstackA1.isEmpty() && itemstackB1.isEmpty() && itemstackC1.isEmpty() )
                {
                    return true;
                }
                else if (!itemstack1.doMaterialsMatch(itemstack) && !itemstackA1.doMaterialsMatch(itemstackA) && !itemstackB1.doMaterialsMatch(itemstackB) && !itemstack1.doMaterialsMatch(itemstackC))
                {
                    return false;
                }
    ...
    public void burn() {
        if (this.canBurn()) {
            ...
            ItemStack itemstack2 = (ItemStack) this.items.get(5);
            ItemStack itemstackA2 = (ItemStack) this.items.get(6);
            ItemStack itemstackB2 = (ItemStack) this.items.get(7);
            ItemStack itemstackC2 = (ItemStack) this.items.get(8);
           ...
           /* CraftItemStack source = CraftItemStack.asCraftMirror(itemstack);
            ...
            if (itemstack2.isEmpty())
            {
                this.items.set(5, itemstack1.cloneItemStack());
                itemstack.subtract(1);
            } 
            ...
            {
                this.items.set(6, itemstackA1.cloneItemStack());
            ...
            {
                this.items.set(7, itemstackB1.cloneItemStack());
            ...
            {
                this.items.set(8, itemstackC1.cloneItemStack());
    ...
    {
        if (enumdirection == EnumDirection.DOWN && i == 4)
        {
            Item item = itemstack.getItem();
    ...
    public Container createContainer(PlayerInventory playerinventory, EntityHuman entityhuman) {
        return new ContainerOnyxFurnace(playerinventory, this);
    }
