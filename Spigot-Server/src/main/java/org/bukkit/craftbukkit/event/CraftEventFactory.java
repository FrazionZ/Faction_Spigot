...
import org.bukkit.event.entity.BigXpEvent;
import org.bukkit.event.entity.CreatureSpawnEvent;
import org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason;
import org.bukkit.event.entity.CreeperPowerEvent;
import org.bukkit.event.entity.DynamiteEvent;
...
public class CraftEventFactory {
    ...
    private static EntityDamageEvent handleEntityDamageEvent(Entity entity, DamageSource source, Map<DamageModifier, Double> modifiers, Map<DamageModifier, Function<? super Double, Double>> modifierFunctions) {
        if (source.isExplosion()) {
            ...
            if (damager == null) {
                ...
                if (damager instanceof org.bukkit.entity.TNTPrimed || damager instanceof org.bukkit.entity.ZTNTPrimed) {
    ...
    public static BigXpEvent callBigXpEvent(Entity entity, int exp) {
        ThrownBigXp bottle = (ThrownBigXp) entity.getBukkitEntity();
        BigXpEvent event = new BigXpEvent(bottle, exp);
        Bukkit.getPluginManager().callEvent(event);
        return event;
    }
    
    
    public static DynamiteEvent callDynamiteEvent(Entity entity, int exp) {
        Dynamite bottle = (Dynamite) entity.getBukkitEntity();
        DynamiteEvent event = new DynamiteEvent(bottle, exp);
        Bukkit.getPluginManager().callEvent(event);
        return event;
    }
    ...
    public static FzBlockExplodeEvent callChestExplodeEvent(World world, BlockPosition pos, IBlockData block, EntityHuman human, ExplosiveType explosionType, ExplosionBlockType chestType) {
    	Player player = ((EntityPlayer) human).getBukkitEntity();
        BlockState blockState = world.getWorld().getBlockAt(pos.getX(), pos.getY(), pos.getZ()).getState();
        blockState.setType(CraftMagicNumbers.getMaterial(block.getBlock()));
        blockState.setRawData((byte) block.getBlock().toLegacyData(block));
        
    	FzBlockExplodeEvent event = new FzBlockExplodeEvent(player, blockState.getBlock(), explosionType, chestType);
    	human.world.getServer().getPluginManager().callEvent(event);
    	return event;
    }
}
